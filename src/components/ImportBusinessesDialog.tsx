...
const BUSINESS_FIELDS = [
  { value: "business_name", label: "Business Name (Required)", required: true },
  { value: "ein", label: "EIN/Tax ID", required: false },
  { value: "industry", label: "Industry", required: false },
  { value: "business_email", label: "Business Email", required: false },
  { value: "business_phone", label: "Business Phone", required: false },
  { value: "website_url", label: "Website URL", required: false },
...
export const ImportBusinessesDialog = ({ open, onOpenChange, onImportComplete }: ImportBusinessesDialogProps) => {
    ...
    Papa.parse(file, {
      ...
      complete: (results) => {
        ...
        const autoMappings: FieldMapping[] = headers.map(header => {
          ...
          if (normalizedHeader.includes("businessname") || normalizedHeader.includes("companyname") || normalizedHeader === "name") {
            businessField = "business_name";
          } else if (normalizedHeader.includes("ein") || normalizedHeader.includes("taxid")) {
            businessField = "ein";
          } else if (normalizedHeader.includes("industry") || normalizedHeader.includes("sector")) {
            businessField = "industry";
          } else if (normalizedHeader.includes("email")) {
            businessField = "business_email";
          } else if (normalizedHeader.includes("phone")) {
            businessField = "business_phone";
          } else if (normalizedHeader.includes("website") || normalizedHeader.includes("url")) {
            businessField = "website_url";
          } else if (normalizedHeader.includes("address") && normalizedHeader.includes("1")) {
            businessField = "address_line1";
          } else if (normalizedHeader.includes("address") && normalizedHeader.includes("2")) {
            businessField = "address_line2";
          ...
          return { csvField: header, businessField };
        });
  ...
  const updateFieldMapping = (csvField: string, businessField: string) => {
    setFieldMappings(prev =>
      prev.map(mapping =>
        mapping.csvField === csvField
          ? { ...mapping, businessField }
          : mapping
      )
    );
  };
  ...
  const validateMappings = (): boolean => {
    const nameMapping = fieldMappings.find(m => m.businessField === "business_name");
    if (!nameMapping) {
      toast({
        title: "Business Name Required",
        description: "You must map a CSV column to the Business Name field",
        variant: "destructive",
      });
      return false;
    }
    ...
    return csvData.slice(0, 5).map(row => {
      ...
      fieldMappings.forEach(mapping => {
        if (mapping.businessField) {
          mappedRow[mapping.businessField] = row[mapping.csvField];
        }
      });
      return mappedRow;
    });
  ...
  const handleImport = async () => {
    ...
    try {
      ...
      const businesses = csvData.map(row => {
        const business: any = {};
        fieldMappings.forEach(mapping => {
          if (mapping.businessField && row[mapping.csvField]) {
            business[mapping.businessField] = row[mapping.csvField].trim();
          }
        });
        return business;
      });
      ...
      if (data.errors.length === 0) {
        toast({
          ...
          description: `${data.imported} businesses imported, ${data.updated} updated`,
        });
        onImportComplete();
      } else {
        toast({
  ...
  const downloadTemplate = () => {
    const template = "business_name,ein,industry,business_email,business_phone,website_url,address_line1,city,state,zip,country\nAcme Corporation,12-3456789,Technology,contact@acme.com,555-0100,https://acme.com,123 Main St,Springfield,IL,62701,US\nGlobal Industries,98-7654321,Manufacturing,info@global.com,555-0200,https://global.com,456 Oak Ave,Chicago,IL,60601,US";
          ...
          {step === 1 && (
            <div className="space-y-6">
              <Alert>
                ...
                <AlertDescription>
                  Upload a CSV file with business information. The file should include at least a business name column.
                </AlertDescription>
              </Alert>
          ...
          {step === 2 && (
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Map your CSV columns to business fields. Business Name is required.
                </AlertDescription>
              </Alert>
              ...
              <div className="flex items-center space-x-2">
                ...
                <Label htmlFor="update-existing" className="text-sm cursor-pointer">
                  Update existing businesses if name matches
                </Label>
              </div>
            </div>
          )}
          ...
          {step === 3 && (
            <div className="space-y-4">
              <Alert>
                ...
                <AlertDescription>
                  Review the first 5 rows before importing {csvData.length} total businesses.
                </AlertDescription>
              </Alert>
                    ...
                    <thead className="bg-muted">
                      <tr>
                        {BUSINESS_FIELDS.filter(f => 
                          fieldMappings.some(m => m.businessField === f.value)
                        ).map(field => (
              ...
              <div className="flex items-center justify-between p-4 rounded-lg bg-muted">
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    {csvData.length} businesses • {updateExisting ? "Will update existing" : "Skip existing"}
                  </p>
                </div>
              </div>
            </div>
                  ...
                  <ScrollArea className="h-48 rounded-lg border p-4">
                    {importResult.errors.map((error, idx) => (
                      <div key={idx} className="mb-2 text-sm">
                        ...
                        <span className="font-medium">{error.businessName}</span>
