...
interface GroupPaymentSetupDialogProps {
  open: boolean;
...
export const GroupPaymentSetupDialog = ({ 
  open, 
  onOpenChange, 
  groupId, 
  groupName 
}: GroupPaymentSetupDialogProps) => {
  ...
  const handleCreateAccount = async () => {
    try {
      ...
      const { data, error } = await supabase.functions.invoke('create-stripe-connect-account', {
        body: { groupId, businessType: 'company' }
      });
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-lg max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Payment Setup - {groupName}</DialogTitle>
        </DialogHeader>
          ...
          <div className="space-y-6">
            ...
            <div className="p-4 bg-muted rounded-lg space-y-3">
              ...
              <ul className="text-sm text-muted-foreground space-y-2">
                ...
                <li className="flex items-start gap-2">
                  <CheckCircle2 className="h-4 w-4 text-green-600 mt-0.5 shrink-0" />
                  100% of donations go to your group (10% platform fee on top)
                </li>
              </ul>
            </div>
                ...
                <div className="flex items-start gap-2">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-green-700 dark:text-green-300 mt-1">
                      Your group is ready to accept payments.
                    </p>
                  </div>
                </div>
              </div>
